...
--   → login_sessions → beneficiaries → account_change_events
--   → transactions → transaction_features → auth_events
--   → sales_points → sales_agents → loan_applications
--   → applicant_declared_profiles → employment_income_profiles
--   → reference_contacts → application_documents
--   → credit_bureau_snapshots → disbursement_accounts
--   → loan_repayment_outcomes
--   → rules → decision_outcomes → rule_hits
--   → alerts → cases → verification_results → fraud_ground_truth
-- ============================================================
...
CREATE TABLE IF NOT EXISTS fraud_sim.simulation_runs (
    ...
    -- completed_at phải sau started_at
    CHECK (completed_at IS NULL OR completed_at >= started_at),
    -- RUNNING: chưa có thời gian kết thúc; COMPLETED/FAILED: bắt buộc có thời gian kết thúc
    CHECK (
        (run_status = 'RUNNING'  AND completed_at IS NULL)
        OR
        (run_status IN ('COMPLETED', 'FAILED') AND completed_at IS NOT NULL)
    )
);
...
CREATE TABLE IF NOT EXISTS fraud_sim.accounts (
    ...
    created_at              TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Số tài khoản là duy nhất trong phạm vi mỗi simulation run (tránh xung đột khi chạy lại cùng seed)
    UNIQUE (simulation_run_id, account_no_hash),
    -- Hỗ trợ composite FK từ login_sessions, transactions, account_change_events
...
CREATE TABLE IF NOT EXISTS fraud_sim.devices (
    ...
    created_at              TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Fingerprint là duy nhất trong phạm vi mỗi simulation run
    UNIQUE (simulation_run_id, device_fingerprint)
);
...
CREATE TABLE IF NOT EXISTS fraud_sim.login_sessions (
    ...
    -- Tọa độ địa lý: dùng để phát hiện impossible travel (đăng nhập từ hai nơi cách xa trong thời gian ngắn)
...
CREATE TABLE IF NOT EXISTS fraud_sim.beneficiaries (
    ...
    -- Một tài khoản không thể thêm cùng một số tài khoản thụ hưởng tại cùng ngân hàng hai lần
    UNIQUE (account_id, beneficiary_account_hash, beneficiary_bank),
    -- Hỗ trợ composite FK từ transactions: đảm bảo beneficiary thuộc đúng account
    UNIQUE (beneficiary_id, account_id)
);
...
CREATE TABLE IF NOT EXISTS fraud_sim.transactions (
    ...
    -- Nếu đối tác là tài khoản nội bộ, tham chiếu trực tiếp để hỗ trợ phát hiện U-turn và layering
    ...
    created_at           TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (account_id, customer_id) REFERENCES fraud_sim.accounts(account_id, customer_id),
    -- Đảm bảo beneficiary thuộc đúng account đang thực hiện giao dịch
...
CREATE TABLE IF NOT EXISTS fraud_sim.auth_events (
    ...
    FOREIGN KEY (account_id, customer_id) REFERENCES fraud_sim.accounts(account_id, customer_id),
    -- Mỗi auth_event phải gắn với đúng một ngữ cảnh: login (session), transaction, hoặc account_change
    CHECK (
...
CREATE TABLE IF NOT EXISTS fraud_sim.sales_agents (
    ...
    created_at                   TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Hỗ trợ composite FK từ loan_applications: đảm bảo agent luôn thuộc đúng điểm bán
...
CREATE TABLE IF NOT EXISTS fraud_sim.loan_applications (
    ...
    is_emulator                BOOLEAN NOT NULL DEFAULT FALSE,
    created_at                 TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Đảm bảo sales_agent_id thuộc đúng sales_point_id (tránh agent "nhảy" điểm bán)
    FOREIGN KEY (sales_agent_id, sales_point_id) REFERENCES fraud_sim.sales_agents(sales_agent_id, sales_point_id),
    -- Hỗ trợ composite FK từ applicant_declared_profiles
...
CREATE TABLE IF NOT EXISTS fraud_sim.applicant_declared_profiles (
    ...
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Đảm bảo application_id và customer_id nhất quán với loan_applications (ngăn khai sai khách hàng)
...
CREATE TABLE IF NOT EXISTS fraud_sim.reference_contacts (
    ...
    -- NOT NULL bắt buộc để UNIQUE (application_id, reference_order) hoạt động đúng (NULL lách được qua UNIQUE)
    reference_order        SMALLINT NOT NULL CHECK (reference_order IN (1, 2)),
    ...
    created_at             TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Mỗi hồ sơ tối đa 2 người tham chiếu, không trùng thứ tự
    UNIQUE (application_id, reference_order),
    -- Không cho nhập trùng số điện thoại trong cùng một hồ sơ
...
CREATE TABLE IF NOT EXISTS fraud_sim.loan_repayment_outcomes (
    ...
    -- paid_on_time / paid_late phân biệt rõ ràng để tránh mơ hồ khi đánh nhãn
    ...
    -- Enum chi tiết hơn boolean: phân biệt lost_contact, refused, invalid thay vì chỉ true/false
    ...
    -- Hai nhãn tách biệt: gian lận (fraud) và nợ xấu tín dụng (credit default) để tránh nhầm khi train model
    ...
    created_at               TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Logic nhất quán DPD: DPD-90 ngụ ý DPD-60, DPD-60 ngụ ý DPD-30
    CHECK (
        (NOT dpd_60_flag OR dpd_30_flag)
        AND (NOT dpd_90_flag OR dpd_60_flag)
    ),
    -- Số kỳ trả đúng hạn không thể vượt tổng số kỳ đến hạn
...
-- Owns: rules, decision outcomes, rule hit details, alerts, cases,
--       verification results, and ground truth labels for evaluation.
-- Creation order: rules -> decision_outcomes -> rule_hits -> alerts -> cases
-- ============================================================
...
-- decision_outcomes phải được tạo trước rule_hits vì rule_hits tham chiếu FK tới bảng này
CREATE TABLE IF NOT EXISTS fraud_sim.decision_outcomes (
    ...
    created_at             TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- Đảm bảo mỗi outcome gắn đúng với transaction hoặc application, không phải cả hai
    CHECK (
...
-- Ghi chi tiết từng rule đã được đánh giá trên mỗi entity: audit trail để giải thích quyết định
...
CREATE TABLE IF NOT EXISTS fraud_sim.alerts (
    ...
    entity_type          TEXT NOT NULL CHECK (entity_type IN (
        'customer', 'account', 'transaction', 'session', 'device',
        'beneficiary', 'change_event', 'loan_application',
        'sales_agent', 'sales_point', 'application_document', 'disbursement_account'
    )),
    entity_id            TEXT NOT NULL,
